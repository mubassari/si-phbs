                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if (isset($user->catatan_admin))
                            <div class="alert alert-warning">{{ $user->catatan_admin }}</div>
                        @endif
                        <x-events.alert-success />
                        <form class="form-horizontal" enctype="multipart/form-data" action="{{ route('profile.update', ['user'  => $user->id]) }}" method="POST">
                            @csrf @method('PATCH')
